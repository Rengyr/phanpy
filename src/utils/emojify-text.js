function emojifyText(text, emojis = []) {
  ...
  emojis.forEach((emoji) => {
    ...
    text = text.replace(
      ...
      `<picture><source srcset="${staticUrl}" media="(prefers-reduced-motion: reduce)"></source><img class="shortcode-emoji emoji" src="${url}" alt=":${shortcode}:" loading="lazy" decoding="async" fetchPriority="low" /></picture>`,
    );
  });
