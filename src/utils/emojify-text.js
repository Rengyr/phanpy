...
function emojifyText(text, emojis = []) {
  ...
  return text.replace(regex, (match) => {
    ...
    return `<picture>${sourceTag}<img class="shortcode-emoji emoji" src="${url}" alt="${escapedShortcode}" title="${shortcode}" loading="lazy" decoding="async" fetchPriority="low" onload="try { this.dataset.isLarger = this.naturalWidth > (this.width * 2) || this.naturalHeight > (this.height * 2) } catch (e) {}" /></picture>`;
  });
}
